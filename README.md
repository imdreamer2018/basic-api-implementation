...
* 添加更新接口
  ...
  接口要求：当userId和rsEventId所关联的User匹配时，更新rsEvent信息
          当userId和rsEventId所关联的User不匹配时，返回4003
          userId为必传字段
...
* 给所有接口添加错误处理：
    ...
    4. post /userRequest 对User进行校验，如果不满足校验规则，则返回 400 {error:"invalid userRequest"}
...
=======
#### 在demo的基础上完成下面需求：
* 重构添加热搜的接口，热搜事件现在增加一个字段:userRequest，用来表示是哪个用户添加的该热搜事件。
